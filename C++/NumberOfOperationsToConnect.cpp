...
#include <vector>
using namespace std;
void DFSutil(int v, vector<bool> &visited, vector<int> &DFS, vector<int> g[])
{
    ...
    for (int i = 0; i < g[v].size(); i++)
    {
        if (!visited[g[v][i]])
        {
            visited[g[v][i]] = true;
            DFS.push_back(g[v][i]);
            DFSutil(g[v][i], visited, DFS, g);
        }
    }
}
int Solution(vector<vector<int>> &connections, int n)
{
...
int main()
{
    vector<vector<int>> vec = {{1, 4}, {0, 3}, {1, 3}, {3, 7}, {2, 7}, {0, 1}, {2, 4}, {3, 6}, {5, 6}, {6, 7}, {4, 7}, {0, 7}, {5, 7}};
    int n = 11;
    Solution(vec, n);
}
//11
